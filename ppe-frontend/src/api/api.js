...
export async function fetchViolations() {
  try {
    const response = await fetch(`${API_BASE_URL}/ppe/violations`);
    if (!response.ok) {
...
export async function fetchWorkers() {
  try {
    const response = await fetch(`${API_BASE_URL}/ppe/workers`);
    if (!response.ok) {
      throw new Error(`HTTP ${response.status}: ${response.statusText}`);
    }
    return await response.json();
...
export async function fetchStatistics() {
  try {
    const response = await fetch(`${API_BASE_URL}/ppe/statistics`);
    if (!response.ok) {
      throw new Error(`HTTP ${response.status}: ${response.statusText}`);
    }
    return await response.json();
...
export async function fetchSettings() {
  try {
    const response = await fetch(`${API_BASE_URL}/ppe/settings`);
    if (!response.ok) {
      throw new Error(`HTTP ${response.status}: ${response.statusText}`);
    }
    return await response.json();
