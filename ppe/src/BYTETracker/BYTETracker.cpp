...
void BYTETracker::predictDS() {}

std::vector<float> BYTETracker::update(std::vector<inference::Detection>& objects, const cv::Mat& frame)
{
	...
	for (unsigned long i = 0; i < output_stracks.size(); i++)
	{
		...
		if (tlwh[2] * tlwh[3] > 20 && !vertical)
		{
			...
			fine.push_back(tlwh[0]);
			fine.push_back(tlwh[1]);
			fine.push_back(tlwh[2]);
			fine.push_back(tlwh[3]);
			fine.push_back(output_stracks[i].score);
			fine.push_back(output_stracks[i].cls);
			fine.push_back(output_stracks[i].track_id);
			
			//std::cout << "track id : " << output_stracks[i].track_id << std::endl;
