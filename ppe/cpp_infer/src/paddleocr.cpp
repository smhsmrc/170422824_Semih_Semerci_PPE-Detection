...
PPOCR::PPOCR(const std::string& det_model_dir,
  ...
  std::cout << "🔧 PaddleOCR özel constructor ile başlatılıyor..." << std::endl;
  
  // FLAGS'leri programatik olarak ayarla
  gflags::SetCommandLineOption("det_model_dir", det_model_dir.c_str());
  gflags::SetCommandLineOption("rec_model_dir", rec_model_dir.c_str());
  gflags::SetCommandLineOption("rec_char_dict_path", rec_char_dict_path.c_str());
  gflags::SetCommandLineOption("use_gpu", use_gpu ? "true" : "false");
  gflags::SetCommandLineOption("cpu_threads", std::to_string(cpu_threads).c_str());
  gflags::SetCommandLineOption("enable_mkldnn", enable_mkldnn ? "true" : "false");
  gflags::SetCommandLineOption("det", det ? "true" : "false");
  gflags::SetCommandLineOption("rec", rec ? "true" : "false");
  gflags::SetCommandLineOption("cls", cls ? "true" : "false");
  gflags::SetCommandLineOption("use_angle_cls", cls ? "true" : "false");
  
  // Varsayılan değerler
  gflags::SetCommandLineOption("gpu_id", "0");
  gflags::SetCommandLineOption("gpu_mem", "4000");
  gflags::SetCommandLineOption("limit_type", "max");
  gflags::SetCommandLineOption("limit_side_len", "960");
  gflags::SetCommandLineOption("det_db_thresh", "0.3");
  gflags::SetCommandLineOption("det_db_box_thresh", "0.6");
  gflags::SetCommandLineOption("det_db_unclip_ratio", "1.5");
  gflags::SetCommandLineOption("det_db_score_mode", "slow");
  gflags::SetCommandLineOption("use_dilation", "false");
  gflags::SetCommandLineOption("use_tensorrt", "false");
  gflags::SetCommandLineOption("precision", "fp32");
  gflags::SetCommandLineOption("rec_batch_num", "6");
  gflags::SetCommandLineOption("rec_img_h", "48");
  gflags::SetCommandLineOption("rec_img_w", "320");
  
  std::cout << "✅ FLAGS ayarlandı:" << std::endl;
  std::cout << "  - det_model_dir: " << det_model_dir << std::endl;
  std::cout << "  - rec_model_dir: " << rec_model_dir << std::endl;
  std::cout << "  - rec_char_dict_path: " << rec_char_dict_path << std::endl;
  std::cout << "  - use_gpu: " << (use_gpu ? "true" : "false") << std::endl;
  std::cout << "  - cpu_threads: " << cpu_threads << std::endl;
  
  // Model'leri başlat
  if (det) {
    // std::cout << "🔍 Detection model yükleniyor..." << std::endl;
    this->detector_.reset(new DBDetector(
        det_model_dir, use_gpu, 0, 4000,
        cpu_threads, enable_mkldnn, "max",
        960, 0.3, 0.6,
        1.5, "slow", false,
        false, "fp32"));
    std::cout << "✅ Detection model yüklendi!" << std::endl;
  }

  if (cls) {
    std::cout << "🔄 Classification model yükleniyor..." << std::endl;
    // cls_model_dir'i ayarlamanız gerekebilir
    this->classifier_.reset(new Classifier(
        "", use_gpu, 0, 4000,
        cpu_threads, enable_mkldnn, 0.9,
        false, "fp32", 1));
    std::cout << "✅ Classification model yüklendi!" << std::endl;
  }
  
  if (rec) {
    std::cout << "📝 Recognition model yükleniyor..." << std::endl;
    this->recognizer_.reset(new CRNNRecognizer(
        rec_model_dir, use_gpu, 0, 4000,
        cpu_threads, enable_mkldnn, rec_char_dict_path,
        false, "fp32", 6,
        48, 320));
    std::cout << "✅ Recognition model yüklendi!" << std::endl;
  }
  
  std::cout << "🎉 PaddleOCR başarıyla başlatıldı!" << std::endl;
}
